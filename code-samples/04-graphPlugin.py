...
class GraphPlugin:
    ...
    def ListCalenderEvents(
        self, user_query: Annotated[str, "the query of the user"]
...
async def main():
    ...
    )  # take the user queries like "List the calendar events", "what was the last meeting", etc.

    client_id = os.getenv(
        "AZURE_OPENAI_CLIENT_ID"
    )  # fill in the client id in the .env file
    tenantId = os.getenv(
        "AZURE_OPENAI_TENANT_ID"
    )  # fill in the tenant id in the .env file
    ...
    finalResult = await kernel.invoke(
        calenderFunction, user_query=user_input
    )  # invoke the function "ListCalendarEvents" with the user query
